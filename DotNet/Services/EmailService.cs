...
namespace Sabio.Services
{
    public class EmailService : IEmailService
    {
        ...
        public async Task SendWorkshopRegistrationConfirmation(WorkshopRegistrationEmail workshopInfo)
        {
            SendGridMessage msg = new SendGridMessage();
            msg.From = new EmailAddress("brijesh@sabio.la");
            msg.Subject = "Workshop Registration Confirmation";
            msg.HtmlContent = GetWorkshopRegistrationConfirmationTemplate(workshopInfo);
            msg.AddTo(new EmailAddress(workshopInfo.UserEmail));

             await SendEmail(msg);
        }

        private string GetWorkshopRegistrationConfirmationTemplate(WorkshopRegistrationEmail workshopInfo)
        {
            var path = _env.WebRootPath + "/EmailTemplate/WorkshopRegistrationConfirmation.html"; 
            string htmlContent = System.IO.File.ReadAllText(path); 
            htmlContent = htmlContent.Replace("{{&&WorkshopImage}}", workshopInfo.WorkshopImage);
            htmlContent = htmlContent.Replace("{{&&UserRegistrationStatus}}", workshopInfo.UserRegistrationStatus);
            htmlContent = htmlContent.Replace("{{&&WorkshopName}}", workshopInfo.WorkshopName);
            htmlContent = htmlContent.Replace("{{&&HostFName}}", workshopInfo.HostFName);
            htmlContent = htmlContent.Replace("{{&&HostLName}}", workshopInfo.HostLName);
            htmlContent = htmlContent.Replace("{{&&DateStart}}", workshopInfo.DateStart.ToString("f"));
            htmlContent = htmlContent.Replace("{{&&LineOne}}", workshopInfo.LineOne);
            htmlContent = htmlContent.Replace("{{&&LineTwo}}", workshopInfo.LineTwo);
            htmlContent = htmlContent.Replace("{{&&City}}", workshopInfo.City);
            //htmlContent = htmlContent.Replace("{{&&State}}", workshopInfo.State); // CAN NOT BE DELETED - FOR FUTURE 
            htmlContent = htmlContent.Replace("{{&&VenueUrl}}", workshopInfo.VenueUrl);
            return htmlContent;
        }

        public async Task SendWorkshopRemovalConfirmation(WorkshopRegistrationEmail workshopInfo)
        {
            SendGridMessage msg = new SendGridMessage();
            msg.From = new EmailAddress("brijesh@sabio.la");
            msg.Subject = "Unregistered From Workshop Confirmation";
            msg.HtmlContent = GetWorkshopRemovalConfirmationTemplate(workshopInfo);
            msg.AddTo(new EmailAddress(workshopInfo.UserEmail));

            await SendEmail(msg);
        }

        private string GetWorkshopRemovalConfirmationTemplate(WorkshopRegistrationEmail workshopInfo)
        {
            var path = _env.WebRootPath + "/EmailTemplate/WorkshopRemovalConfirmation.html";
            string htmlContent = System.IO.File.ReadAllText(path);
            htmlContent = htmlContent.Replace("{{&&WorkshopImage}}", workshopInfo.WorkshopImage);
            htmlContent = htmlContent.Replace("{{&&UserRegistrationStatus}}", workshopInfo.UserRegistrationStatus);
            htmlContent = htmlContent.Replace("{{&&WorkshopName}}", workshopInfo.WorkshopName);
        ...
        public async Task<bool> ContactUs(ContactUsAddRequest model)
        {
            ...
            SendGridMessage msg = new SendGridMessage();
            {
                ...
                msg.HtmlContent = System.IO.File.ReadAllText(path).Replace("{FirstName}", model.FirstName)
        ...
        public async Task<bool> SubmitFaq(FaqsSubmitRequest model)
        {
            SendGridClient client = GetMailClient();
            var email = new EmailAddress("stephaniemzavala@gmail.com");
            var path = _env.WebRootPath + "/EmailTemplate/FaqSubmission.html";
            SendGridMessage msg = new SendGridMessage();
            {
                msg.From = new EmailAddress(model.Email, model.Name);
                msg.Subject = model.Question;
                msg.HtmlContent = System.IO.File.ReadAllText(path).Replace("{Name}", model.Name).Replace("{Question}", model.Question).Replace("{Email}", model.Email);
            }
