...
function User(props) {
    ...
    const getUserProfile = () => {
        if (aUser.id) {
            userDashBoardService.getUserProfile(aUser.id).then(onGetUserProfileSuccess).catch(onGetUserProfileError);
        }
    };
            ...
            <Card>
                <Card.Body>
                    <Dropdown className="float-end" align="end">
                        ...
                        <Dropdown.Menu>
                            <Dropdown.Item
                                // key={aUser.id}
                                // onClick={() => {
                                //     navigate(`/users/update?${aUser.id}`);
                                // }}
                    ...
                    <div className="mt-3 text-center">
                        <img src={aUser.avatarUrl} alt="avatar" className="img-thumbnail avatar-lg rounded-circle" />
                        <h4>{aUser.name}</h4>
                        <Button
                            onClick={() => {
                                navigate(`/chat`);
