...
import userIcon from './userIconn.png'
...
const AttendanceDropdown = () => {
    ...
    const mapUserItems = (user) => { 
        return (
            ...
            <tbody>
             <tr key={user.id}>
                <td><img src={user.avatarUrl} width="45px" height="auto" className="rounded" alt="user" /></td>
     ...
     <React.Fragment>
        ...
        <Card id="renderPage">
         <Card.Body>
                ...
                {show && <div className="row Counter">
                    <div className="column">
                            <div className="card countingC">
                                <p><img className="user-i" src={userIcon} alt="icon"></img></p>
                                     <h3>{show && pageData.startCounter}</h3>
                                    <p>{show && " Attendees" }</p>
                </div>
                </div>
                </div>} 
                ...
                <Table className="mb-0" hover>
                   ...
                   {show && pageData.user.map(mapUserItems)}
                </Table>
                {show ? true :<div className='placeHolder-text' style={{size:"15px", color:"rgb(161, 161, 161)", display:"flex", justifyContent:"center", margin:"30px"}}>Users enrolled in your workshop will appear here..</div>}
                </Card.Body>
                </Card>
                 </div>
                </div>
